...
CUDA 4.1 has been [released](http://developer.nvidia.com/cuda-toolkit-41), and with it, and they\'ve added support for writing to 3D surfaces. And
thanks to some pointers from some very helpful Nvidia engineers (thanks [Gernot](http://www.mpi-inf.mpg.de/~gziegler/)!), I was able to write to a 3D OpenGL texture with a CUDA kernel, without having to copy any data between the host and the device.
...
The sample uses `cudaMalloc3DArray` to directly allocate
data for the 3D surfaces, so it doesn\'t show the process for 3D surface
writes in which the allocation has occurred by creating an OpenGL
texture. Fortunately, that takes just a few extra steps.

## The Steps

1.  [Create an OpenGL 3D Texture](https://rauwendaal.net/2011/12/#step1)
2.  [Register the texture as an \"image\" with CUDA](https://rauwendaal.net/2011/12/#step2)
3.  [Map the \"image\" to a CUDA graphics resource](https://rauwendaal.net/2011/12/#step3)
4.  [Get a `cudaArray` pointer from the resource](https://rauwendaal.net/2011/12/#step4)
5.  [Pass the `cudaArray` pointer to the device](https://rauwendaal.net/2011/12/#step5)
6.  [Bind the `cudaArray` to a globally scoped CUDA surface](https://rauwendaal.net/2011/12/#step6)
7.  [Call a CUDA kernel](https://rauwendaal.net/2011/12/#step7)
8.  [Write to the surface using `surf3Dwrite`](https://rauwendaal.net/2011/12/#step8)
9.  [Unmap the resource](https://rauwendaal.net/2011/12/#step9)
10. [Unregister the texture](https://rauwendaal.net/2011/12/#step10)

### Step 1: Create an OpenGL 3D texture

Hopefully most people know how to do this, just watch out that you
are using a texture format that is CUDA compatible, I\'m not entirely
sure all which textures are supported, but [this forum post](http://forums.nvidia.com/index.php?showtopic=164987) shows a couple that definitely work.
